...
class CustomTrainer(train.BaseTrainer):
    ...
    def __init__(self, args):
        ...
        self.tracker = other_tools.EpochTracker(["rec", "contact", "vel", "foot", "ver", "ver_vel", "ver_acc", "com", "kl", "acc", "trans", "transv", "loss"], 
    ...
    def train(self, epoch):
        ...
        for its, dict_data in enumerate(tqdm(self.train_loader,desc=f'train epoch {epoch}')):
            ...
            in_tar_pose = torch.cat((tar_pose, tar_contact), dim=-1)
            ...
            foot_loss = self.vel_loss(model_foot_v, torch.zeros_like(model_foot_v))
            self.tracker.update_meter("foot", "train", foot_loss.item())
            g_loss_final += foot_loss * self.args.foot_weight
    ...
    def val(self, epoch, train_its):
        ...
        with torch.no_grad():
            for its, dict_data in enumerate(tqdm(self.val_loader,desc=f'val epoch {epoch} iter {train_its}')):
                ...
                model_foot_v[~static_idx] = 0
                foot_loss = self.vel_loss(model_foot_v, torch.zeros_like(model_foot_v))
                self.tracker.update_meter("foot", "val", foot_loss.item())
                g_loss_final += foot_loss * self.args.foot_weight
